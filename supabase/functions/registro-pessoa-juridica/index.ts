...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { empresa, responsavel, plano_id, codigo_cupom } = body;

    console.log("Starting PJ registration for:", responsavel.email);
    ...
    if (!empresa.razao_social || !empresa.cnpj || !empresa.email || !responsavel.nome || !responsavel.email || !responsavel.senha) {
      return new Response(
    ...
    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
      ...
      user_metadata: { nome: responsavel.nome },
    });
    ...
    // 2. Create the imobiliaria (simplified for PJ individual)
    ...
    if (imobError) {
      console.error("Empresa error:", imobError);
      // Rollback: delete the user
      await supabaseAdmin.auth.admin.deleteUser(userId);
      return new Response(
        JSON.stringify({ error: "Erro ao criar empresa: " + imobError.message }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    console.log("Empresa created:", imobData.id);
    ...
    console.log("PJ Registration completed successfully");
