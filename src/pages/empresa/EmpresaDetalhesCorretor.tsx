...
export default function EmpresaDetalhesCorretor() {
  ...
  async function fetchData() {
    if (!userId || !imobiliariaId) return;
    ...
    try {
      ...
      if (membrosData) {
        ...
        const liderDeAlguma = equipesDoCorretor.some(e => e.lider_id === userId);
        setIsLider(liderDeAlguma);
      }
    ...
    <ImobiliariaLayout>
      <div className="space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row gap-6">
              <UserAvatar
                ...
                name={profile.nome}
                size="lg"
                className="h-24 w-24 text-2xl mx-auto md:mx-0"
              />
              <div className="flex-1 space-y-3 text-center md:text-left">
                ...
                {equipes.length > 0 && (
                  <div className="flex flex-wrap justify-center md:justify-start gap-2">
                    {equipes.map(equipe => (
                      <EquipeBadge key={equipe.id} nome={equipe.nome} cor={equipe.cor} />
                    ))}
                  </div>
                )}
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          {surveyEnabled && surveyResponses.length > 0 && (
            <Card>
              <CardContent className="pt-4">
                <div className="flex items-center gap-2 text-muted-foreground text-sm">
                ...
                <p className={`text-2xl font-bold mt-1 ${getRatingColor(mediaGeral as string)}`}>
                  {mediaGeral}/5
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Registros de Visita ({fichas.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                {fichas.length === 0 ? (
